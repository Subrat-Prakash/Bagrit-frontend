...
export function ProductThree() {
  ...
  useEffect(() => {
    ...
    const fetchWasteReqData = async () => {
      try {
        const response = await axios.get("https://bagrit-backend.onrender.com/getInnovativeProd", {
          withCredentials: true,  // Ensure cookies are sent
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${localStorage.getItem("token")}` // Send token if stored in localStorage
          }
        });
